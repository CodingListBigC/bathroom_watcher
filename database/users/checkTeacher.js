            ...
            if (currentRow.teacherStudent === 1) {
                ...
                rl.question('Is this user a teacher? (1 for yes, 0 for no): ', (answer) => {
                    if (answer === '1' || answer === '0') {
                        if (answer === '1') {
                            db.get(`SELECT username FROM teachers WHERE username = ?`, [currentRow.username], (err, row) => {
                                if (err) {
                                    console.error('Error checking teacher record:', err.message);
                                    processRow(index + 1);
                                } else if (row) {
                                    console.log(`Username ${currentRow.username} already exists in the teachers table.`);
                                    processRow(index + 1);
                                } else {
                                    db.run(
